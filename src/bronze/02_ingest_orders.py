#!/usr/bin/env python3
"""
Bronze ingestion entry point: orders.csv -> bronze_orders Delta table.

Databricks usage:
    %run ./bronze_ingest
    %run ./02_ingest_orders
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest orders CSV into Bronze Delta.")
    parser.add_argument(
        "--source-base-path",
        help="Directory containing orders.csv (overrides PIPELINE_SOURCE_BASE_PATH).",
    )
    ...
    parser.add_argument(
        "--table-name",
        help="Bronze table name (default: bronze_orders).",
    )
    parser.add_argument(
        "--write-mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    overrides = {
        ...
        "bronze_orders_table": args.table_name,
    ...
    try:
        ...
        result = ingest_entity(
            entity="orders",
            source_filename=config.orders_csv,
            target_table_name=config.bronze_orders_table,
            spark=spark,
            config=config,
        )
        logging.info(
            "orders Bronze ingest succeeded: %s rows written to %s",
            result.row_count,
            result.target_table,
        )
        return 0
    except BronzeIngestError as exc:
        logging.error("orders Bronze ingest failed: %s", exc)
        return 1
