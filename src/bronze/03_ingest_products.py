#!/usr/bin/env python3
"""
Bronze ingestion entry point: products.csv -> bronze_products Delta table.

Databricks usage:
    %run ./bronze_ingest
    %run ./03_ingest_products
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest products CSV into Bronze Delta.")
    parser.add_argument(
        "--source-base-path",
        help="Directory containing products.csv (overrides PIPELINE_SOURCE_BASE_PATH).",
    )
    ...
    parser.add_argument(
        "--table-name",
        help="Bronze table name (default: bronze_products).",
    )
    parser.add_argument(
        "--write-mode",
...
def main(argv: list[str] | None = None) -> int:
    ...
    overrides = {
        ...
        "bronze_products_table": args.table_name,
    ...
    try:
        ...
        result = ingest_entity(
            entity="products",
            source_filename=config.products_csv,
            target_table_name=config.bronze_products_table,
            spark=spark,
            config=config,
        )
        logging.info(
            "products Bronze ingest succeeded: %s rows written to %s",
            result.row_count,
            result.target_table,
        )
        return 0
    except BronzeIngestError as exc:
        logging.error("products Bronze ingest failed: %s", exc)
        return 1
